...
assert(world_region(player.x,player.z)=='archive')
assert(world_exposure({x=-20,y=0,z=15})<0.45,'archive is no longer dark')
agent_look_at(-21,1.4,15)
engine_snapshot('replays/shots/m4-dark-branch.png')
print('SMOKE dark branch: built archive reached; low available light checked')
agent_walk_to(-8,17) agent_walk_to(0,17)
agent_walk_to(0,8) agent_look_at(-2,0.65,-1)
assert(world_exposure(creatures[1])>0.85,'lake needs flash despite open roof')
engine_snapshot('replays/shots/m4-lake-shore.png')
agent_tap('space')
